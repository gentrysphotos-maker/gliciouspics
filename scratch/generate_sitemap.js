...
if (products.panoramas) {
  products.panoramas.forEach(p => {
    urls.push(`https://gliciouspics.com/pages/product-panorama.html?id=${p.id}`);
  });
}
...
if (products.aerial) {
  products.aerial.forEach(p => {
    urls.push(`https://gliciouspics.com/pages/product-aerial.html?id=${p.id}`);
  });
}
...
if (products.standard) {
  products.standard.forEach(p => {
    urls.push(`https://gliciouspics.com/pages/product-standard.html?id=${p.id}`);
  });
}
